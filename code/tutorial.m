...
if compTraj
  ...
  if value <= 0 %if initial state is in BRS/BRT
    ...
    s.SizeData = 70;
    title('The reachable set at the end and x_{init}')
    %hold on;
    %plot3(traj(1,:),traj(2,:), traj(3,:), 'kv');
    hold off
    ...
    % Plot valueFunction surf
    % define dim to keep [x, y, theta] -> [1, 1, 0] : keep x,y, ignore theta
    dimsToKeep = [1 1 0];
    % define theta slice position
    slicePoint = [-pi];
    
    % use proj to get 2D grid and data
    [g2D, data2D] = proj(g, data(:,:,:,1), ~dimsToKeep, slicePoint);
    
    figure(7);
    mesh(g2D.vs{1}, g2D.vs{2}, data2D');
    hold on;
    % draw the level set curve
    zlim([-2, 2]); % adjust the value range
    contour3(g2D.vs{1}, g2D.vs{2}, data2D', [0 0], 'r', 'LineWidth', 3);
    title('Projected Value Function using proj()');
  else
